...
const EditForm = ({ addClassroom, editClassroom, classroomToEdit }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [moduleName, setModuleName] = useState(
    classroomToEdit?.moduleName || ""
  );
  const [lessonName, setLessonName] = useState(
    classroomToEdit?.lessonName || ""
  );
  const [fromCapacity, setFromCapacity] = useState(
    classroomToEdit?.fromCapacity || ""
  );
  const [toCapacity, setToCapacity] = useState(
    classroomToEdit?.toCapacity || ""
  );
  const [classStatus, setClassStatus] = useState(
    classroomToEdit?.classStatus || ""
  );
  const [description, setDescription] = useState(
    classroomToEdit?.description || ""
  );
  const [file, setFile] = useState(classroomToEdit?.file || null);
  ...
  const handleAddOrEditClass = (e) => {
    ...
    if (classroomToEdit) {
      editClassroom(classroomToEdit.moduleName, newClass);
    } else {
            ...
            <div className="relative bg-white rounded-lg shadow">
              <div className="flex items-center justify-between p-4 border-b rounded-t">
                <h3 className="text-lg font-semibold text-gray-900">
                  {classroomToEdit ? "Edit Class Details" : "Add Class Details"}
                </h3>
                <button
                  type="button"
              ...
              <form className="p-4" onSubmit={handleAddOrEditClass}>
                <div className="mb-4">
                  <label
                    htmlFor="moduleName"
                ...
                <div className="flex justify-end">
                  ...
                  >
                    {classroomToEdit ? "Save Changes" : "Add"}
                  </button>
                  <button
                    type="button"
                    className="ml-2 text-gray-700 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 font-medium rounded-lg text-sm px-5 py-2.5"
                    onClick={() => setIsOpen(false)}
                  >
                    Cancel
                  </button>
